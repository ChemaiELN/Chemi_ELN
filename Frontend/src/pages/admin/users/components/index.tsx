...
import {
  ...
  Modal, Form, Popconfirm, Switch, Tooltip, Divider,
} from 'antd'
import {
  HomeOutlined, SearchOutlined,
  EditOutlined, StopOutlined, CheckOutlined as AntCheckOutlined,
...
const MORE_COLUMNS_TOOLTIP = 'Shows: Contact, Site, Dashboard reference, Flags'
...
export default function AdminUsersPage() {
  ...
  const coreColumns: ColumnsType<UserRow> = [
    ...
    { title: 'DISPLAY NAME', dataIndex: 'displayName', key: 'displayName' },
    {
      ...
      render: (v: string) => (
        <Tooltip title={roleNames[v] ?? v}>
          <Tag color={ROLE_COLOR[v] ?? 'default'} style={{ fontWeight: 600, cursor: 'default' }}>{v}</Tag>
        </Tooltip>
      ),
    },
    ...
    { title: 'EMAIL', dataIndex: 'email', key: 'email', className: styles.emailCell, ellipsis: true },
    {
      ...
      render: (v: boolean) => (
        <StatusTag status={v ? 'ACTIVE' : 'INACTIVE'} label={v ? 'Active' : 'Inactive'} />
      ),
    },
  ]
        ...
        <Space size={4}>
          ...
          {record.allowSettingsUpdate && (
            <Tooltip title="Can edit global/CRD settings">
              <SettingOutlined style={{ color: '#5aa3a1', fontSize: 13 }} />
            </Tooltip>
          )}
          {!record.mustResetPassword && !record.allowSettingsUpdate && '—'}
        </Space>
      ),
    },
  ...
  const actionColumn: ColumnsType<UserRow> = [
    {
      ...
      render: (_: unknown, record: UserRow) => (
        <Space size={4}>
          ...
          >
            <Button type="text"
              icon={record.active ? <StopOutlined /> : <AntCheckOutlined />}
              size="small"
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
        ...
        <Form.Item name="allow_settings_update" label="Allow Settings Update"
          ...
          <Switch size="small"
            style={{ '--ant-switch-color': '#5aa3a1' } as React.CSSProperties} />
        </Form.Item>
          ...
          <div className={styles.tableCard}>
            <div className={styles.tableCardHeader}>
              ...
              <div className={styles.tableCardFilters}>
                <Tooltip title={MORE_COLUMNS_TOOLTIP}>
                  <div className={styles.moreColumnsToggle}>
                    <span className={styles.filterLabel}>More columns</span>
                    <Switch size="small" checked={showExtra} onChange={setShowExtra}
                      style={showExtra ? { background: '#5aa3a1' } : {}} />
                  </div>
                </Tooltip>
