...
const FIELD_TYPES = ['text', 'number', 'textarea', 'select', 'date', 'checkbox', 'file'] as const
    ...
    <div className={styles.screenCard}>
      ...
      <div className={styles.screenCardHeader}>
        ...
        <div className={styles.screenTitleGroup}>
          <OverflowTooltip title={screen.title}>
            <Input
              size="small"
          ...
          <OverflowTooltip title={screen.key}>
            <Input
              size="small"
              placeholder="key (auto)"
              value={screen.key}
        ...
        <div className={styles.screenMeta}>
          ...
          />
          <Tooltip title="Require e-signature when this screen is submitted">
          ...
          <Tooltip title="Allow file attachments on this screen">
            <label className={styles.screenToggle}>
              <Switch
                size="small"
          ...
          {screen.fields.length === 0 ? (
            ...
            <div className={styles.fieldsList}>
              <div className={styles.fieldsHeader}>
                ...
                <span className={styles.fieldsHeaderLabel}>Required</span>
                <span className={styles.fieldsHeaderLabel} />
              </div>
...
const WorkflowTemplateConfigurePage: React.FC = () => {
  ...
  const restoreState = location.state as { restoreDefinition?: Record<string, unknown>; fromVersion?: number } | null
  ...
  const snapshotRef = useRef<{ meta: string; def: string } | null>(null)
  const savedFlashTimer = useRef<ReturnType<typeof setTimeout> | null>(null)
  const [dirty, setDirty] = useState(false)
  const [savedFlash, setSavedFlash] = useState(false)

  const takeSnapshot = useCallback((metaValues: Record<string, unknown>, defState: TemplateDef) => {
    snapshotRef.current = {
      meta: JSON.stringify(metaValues),
      def: JSON.stringify(defState),
    }
    setDirty(false)
  }, [])

  const checkDirty = useCallback((defState: TemplateDef) => {
    if (!snapshotRef.current) return
    const meta = JSON.stringify(metaForm.getFieldsValue(true))
    const defStr = JSON.stringify(defState)
    setDirty(meta !== snapshotRef.current.meta || defStr !== snapshotRef.current.def)
  }, [metaForm])
  ...
  const setDefTracked = useCallback((updater: TemplateDef | ((prev: TemplateDef) => TemplateDef)) => {
    setDef(prev => {
  ...
  useEffect(() => {
    if (isNew) {
      ...
      takeSnapshot({ is_active: true }, { sections: [] })
      return
    }
    setLoading(true)
    getWorkflowTemplate(id!)
      .then(t => {
        setTemplate(t)
        const parsed = restoreState?.restoreDefinition
          ? parseDef(restoreState.restoreDefinition)
          : parseDef(t.definition)
        setDef(parsed)
        const metaValues = {
          name:        t.name,
          slug:        t.slug,
        ...
        metaForm.setFieldsValue(metaValues)
        takeSnapshot(metaValues, parsed)
      })
      .catch(() => message.error('Failed to load template'))
      .finally(() => setLoading(false))
  }, [id, isNew, metaForm, takeSnapshot])

  // ── Definition helpers ────────────────────────────────────────
  const addSection = () => setDefTracked(d => ({ sections: [...d.sections, emptySection()] }))

  const updateSection = useCallback((i: number, sec: SectionDef) =>
    setDefTracked(d => { const s = [...d.sections]; s[i] = sec; return { sections: s } }), [setDefTracked])

  const removeSection = (i: number) =>
    setDefTracked(d => ({ sections: d.sections.filter((_, idx) => idx !== i) }))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        const updated = await updateWorkflowTemplate(id!, {
          name:        values.name,
          description: values.description,
          category:    values.category,
    ...
    <div className={styles.page}>
      ...
      <div className={styles.body}>
        ...
        <main className={styles.main}>
          ...
          {restoreState?.fromVersion && (
            <div style={{
              background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 6,
              padding: '8px 14px', fontSize: 12, color: '#92400e', display: 'flex', gap: 8, alignItems: 'center',
            }}>
              <span>⚠️</span>
              <span>
                You are viewing <strong>v{restoreState.fromVersion}</strong> of this template.
                The fields below reflect that version — edit as needed and click <strong>Save Changes</strong> to apply as the new current version.
              </span>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className={styles.editorLayout}>
              ...
              {/* ── Left: metadata panel ── */}
              <div className={`${styles.metaPanel} ${styles.scrollPanel}`}>
                ...
                >
                  ...
                  >
                    <Input
                      ...
                      onChange={e => {
                        if (isNew) {
                          metaForm.setFieldValue('slug', toSlug(e.target.value))
                          checkDirty(def)
                        }
                      }}
                    />
                  ...
                  <Form.Item
                    ...
                    label={(
                      <span className={styles.slugLabel}>
                        Slug
                        {!isNew && <Lock size={14} strokeWidth={2} className={styles.slugLockIcon} aria-hidden />}
                      </span>
                    )}
                    rules={[{ required: true, message: 'Required' }]}
                    extra={isNew ? 'Auto-generated from name. Cannot be changed after creation.' : 'Auto-generated and locked after creation.'}
                  >
                    <Input
                      placeholder="e.g. adc_stability"
                      disabled={!isNew}
                      readOnly={!isNew}
                      className={!isNew ? styles.slugInputReadOnly : undefined}
                    />
                  </Form.Item>
              ...
              {/* ── Right: field builder ── */}
              <div className={`${styles.builderPanel} ${styles.scrollPanel}`}>
