...
export default function UnlockExperimentsPage() {
  ...
  const handleApprove = async (values: { review_note?: string }) => {
    ...
    try {
      const updated = await reviewUnlockRequest(approveTarget.id, 'approve', values.review_note)
  ...
  const handleReject = async (values: { review_note: string }) => {
    ...
    try {
      const updated = await reviewUnlockRequest(rejectTarget.id, 'reject', values.review_note)
      setUnlockRows(prev => prev.map(r => r.id === updated.id ? updated : r))
  ...
  const unlockColumns: TableColumnsType<UnlockRequestResponse> = [
    ...
    },
    {
      ...
      render: (_: unknown, record: UnlockRequestResponse) => (
        <Text
          style={{ fontSize: 12, color: '#5aa3a1', cursor: 'pointer', fontWeight: 600 }}
  ...
  const expColumns: TableColumnsType<ExperimentSummary> = [
    ...
    },
    {
      ...
      render: (v: string) => (
        <span style={{ fontWeight: 600, fontSize: 12, color: '#5aa3a1' }}>{v}</span>
      ),
    },
    ...
    {
      title: 'Created By', dataIndex: 'created_by', key: 'created_by', width: 140,
      render: (v: string | null) => v ?? '—',
    },
    {
