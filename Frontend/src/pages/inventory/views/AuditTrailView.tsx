...
import { Table, Input, Select, DatePicker, Typography, message } from 'antd'
...
function formatAuditTimestamp(v: string | null | undefined): string {
  if (!v) return '—'
  const d = dayjs(v)
  if (d.isValid()) return d.format('DD-MMM-YYYY HH:mm')
  const legacy = dayjs(v, 'DD MMM YYYY (HH:mm:ss)', true)
  if (legacy.isValid()) return legacy.format('DD-MMM-YYYY HH:mm')
  if (/^\d{4}-\d{2}-\d{2}$/.test(v)) return `${dayjs(v).format('DD-MMM-YYYY')} —`
  return v
}
  ...
  const columns: ColumnsType<AuditTrailEntry> = [
    {
      ...
      render: v => <span className={styles.dimCell}>{formatAuditTimestamp(v)}</span>,
    },
    {
  ...
  return (
    <div>
      <div className={styles.masterPageTitle}>
        ...
        <InventoryCountBadge count={total} />
      </div>
