        ...
        <Space size={4}>
          ...
          <Popconfirm title="Delete this mapping?" onConfirm={() => handleDelete(row.id)} okText="Delete" okButtonProps={{ danger: true }}>
            <Tooltip title="Delete">
              <Button size="small" icon={<DeleteOutlined />} danger className={styles.rowDeleteBtn} />
            </Tooltip>
          </Popconfirm>
        </Space>
      ),
    ...
    <div>
      <div className={styles.masterCard}>
        <div className={styles.masterCardHeader}>
          ...
          <InventoryAddButton onClick={openAdd}>Add Mapping</InventoryAddButton>
        </div>
        <Table<ManufacturerMapping>
          rowKey="id" size="small" loading={loading}
          dataSource={rows} columns={columns}
          className={`${styles.masterTable} ${styles.mappingTable}`}
