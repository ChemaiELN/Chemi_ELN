"""
Patch adc-synthesis-v2 template: convert free-form detail/remarks/notes table
columns from type='text' to type='rich_text', then sync notebook snapshots.
"""
import sys, copy
sys.path.insert(0, ".")
...
# Column keys that should become rich_text
RICH_TEXT_COL_KEYS = {"details", "detail", "remarks", "notes"}
        ...
        for scr in sec.get("screens", []):
            for f in scr.get("fields", []):
                ...
                for col in f.get("columns", []):
                    if col.get("key") in RICH_TEXT_COL_KEYS and col.get("type") == "text":
                        col["type"] = "rich_text"
                        print(f"  [OK] {scr['key']}.{f['key']}.{col['key']} -> rich_text")
                        count += 1
    return defn, count
...
def run():
    with engine.begin() as conn:
        ...
        print(f"  {n} column(s) updated to rich_text")
