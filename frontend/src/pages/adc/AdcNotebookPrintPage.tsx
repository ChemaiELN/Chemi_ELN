...
function formatValue(val: unknown, type: string): string {
  if (val === null || val === undefined || val === '') return EMPTY_VALUE_TEXT
  ...
  if (type === 'action' && typeof val === 'object') {
    ...
    const parts = [`Submitted by ${v.submitted_by ?? EMPTY_VALUE_TEXT}`, when].filter(Boolean)
...
export default function AdcNotebookPrintPage() {
  ...
  if (loadingNb || loadingExp) {
    return <div className="p-6 h-screen"><BrandSpinner fullScreen={false} label="Preparing notebook for printing…" /></div>
  }
            ...
            {exp?.status === 'APPROVED' && (
              <div className="mb-4 border border-gray-200 rounded p-3 bg-gray-50 text-xs grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{exp.submitted_by ?? EMPTY_VALUE_TEXT}</p>
                  <p className="text-gray-400">{exp.submitted_at ? new Date(exp.submitted_at).toLocaleString() : ''}</p>
                </div>
                <div>
                  <p className="text-gray-500">Approved by</p>
                  <p className="font-medium">{exp.approved_by ?? EMPTY_VALUE_TEXT}</p>
                              ...
                              <tr key={ri}>
                                {cols.map(c => (
                                  <td key={c} className="border border-gray-200 px-2 py-1">
                                    {row[c] !== null && row[c] !== undefined ? String(row[c]) : EMPTY_VALUE_TEXT}
                                  </td>
                                ))}
                              </tr>
                            ))}
