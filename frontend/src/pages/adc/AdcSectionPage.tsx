...
import { Button, Tag, Tabs, Alert, Modal, Input, Tooltip, message } from 'antd'
import {
...
export default function AdcSectionPage() {
  ...
  // Gates both the New Experiment action and the auto-create-on-first-visit below.
  const canCreateExperiment = useCan('adc.experiment.create')
  const canEditExperiment   = useCan('adc.experiment.edit')
  ...
  // Status says "is this currently changeable", the privilege says "may you" —
  // both must hold. `frozen_at` overrides both: set when this experiment's
  // Notebook is closed (while not yet Approved) or deactivated — see
  // notebooks.routes.ts's Close/Reopen/Deactivate endpoints.
  const editable = canEditExperiment && !exp?.frozen_at && (exp ? EDITABLE_STATUSES.has(exp.status) : false)
  ...
  if (isLoading || !nb) {
    return <div className="p-6 h-[60vh]"><BrandSpinner fullScreen={false} label="Loading section…" /></div>
  }
  if (!section) {
    return (
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-violet-600 mb-4 transition-colors"
      >
      ...
      <div className="glass-card rounded-2xl p-4 mb-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button
              ...
              type="text"
            />
            {exp?.status === 'DRAFT' && canSubmit && !exp?.frozen_at && (
              <Button
                type="primary"
                size="small"
                style={BTN_32}
            ...
            {exp?.status === 'SUBMITTED' && (canApprove || canReject) && !exp?.frozen_at && (
              <>
                {canApprove && (
                  <Button
                    type="primary"
            ...
            {(exp?.status === 'APPROVED' || exp?.status === 'REJECTED') && canUnlock && !exp?.frozen_at && (
              <Button
                size="small"
                style={BTN_32}
                icon={<Unlock size={13} />}
        ...
        {exp?.frozen_at && (
          <Alert
            message="This experiment is frozen — its Notebook is closed or deactivated, so it can no longer be edited, submitted, or reviewed."
            type="warning"
            showIcon
            className="mt-3"
          />
