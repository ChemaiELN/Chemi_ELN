...
function DoneReviewedSignatureField({
  ...
  // Display only — who may sign is decided by the privileges below.
  ...
  // hardcoded, so both screens and the API now agree on one source of truth.
  const maySignDone     = useCan('adc.experiment.sign_done')
  const maySignReviewed = useCan('adc.experiment.sign_checked')

  const canSignDone     = !disabled && !doneBy && maySignDone
  const canSignReviewed = !disabled && !!doneBy && !reviewedBy && maySignReviewed
      ...
      <div className="flex-1 min-w-[220px] border border-slate-200 rounded-lg p-3">
        {reviewedBy ? (
          ...
          <>
            ...
            {!disabled && !!doneBy && !maySignReviewed && (
              <p className="text-[11px] text-slate-400 mt-1.5">You do not have permission to review.</p>
            )}
          </>
        )}
      </div>
...
export default function FieldRenderer({
  ...
  if (type === 'action' && field.action_type === 'submit_to_ad') {
    type AdResultTest = {
      testType?: string; testSubtype?: string; status?: string
      remarks?: string; assignedToName?: string
    }
    type AdResult = {
      form_no?: string; status?: string; tests?: AdResultTest[]
      analysis_remarks?: string; certification_remarks?: string; received_at?: string
    }
    const submission = value as {
      submitted?: boolean; submitted_at?: string; sample_qty?: string; sku_pack_id?: string
      ad_atr_id?: string; ad_form_no?: string; ad_result?: AdResult
    } | undefined
    const submitted = !!submission?.submitted
    const adResult = submission?.ad_result
    ...
    const handleSubmitToAd = async () => {
      ...
      // The Unit field next to Sample Qty (dropdown, key ending in `_unit`) —
      // when present, tells the backend what unit `sample_qty` was entered in
      // so it can convert to the batch's own stored unit before deducting.
      // Optional for back-compat with templates that don't have this field.
      const sampleUnitKey = (screenFields ?? []).find(f => f.type === 'dropdown' && f.key.endsWith('_unit'))?.key
      const sampleUnit = sampleUnitKey ? (contextData?.[sampleUnitKey] as string | undefined) : undefined
      ...
      <div className="col-span-3">
        ...
        {submitted && submission?.submitted_at && (
          <p className="text-xs text-slate-400 mt-1.5">
            ...
            {submission.ad_form_no ? ` — AD ref ${submission.ad_form_no}` : ''}
          </p>
        )}
        ...
        {submitted && !adResult && (
          <p className="text-xs text-amber-600 mt-1 flex items-center gap-1">
            ...
            Awaiting results from AD{submission?.ad_form_no ? ` (${submission.ad_form_no})` : ''}
          </p>
        )}

        {adResult && (
          <div className="mt-2 border border-slate-200 rounded-md p-3 bg-slate-50 max-w-md">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-slate-600">
                AD Results {adResult.form_no ? `— ${adResult.form_no}` : ''}
              </p>
              {adResult.status && (
                <span
                  className={
                    'text-[11px] font-medium px-2 py-0.5 rounded ' +
                    (adResult.status === 'CERTIFIED'
            ...
            {adResult.tests && adResult.tests.length > 0 && (
              <table className="w-full text-xs mb-2">
                <thead>
                  <tr className="text-slate-400 text-left">
                    <th className="font-medium pb-1">Test</th>
                    <th className="font-medium pb-1">Status</th>
                    <th className="font-medium pb-1">Analyst</th>
                  </tr>
                </thead>
                <tbody>
                  {adResult.tests.map((t, i) => (
                    <tr key={i} className="border-t border-slate-200">
                      <td className="py-1 pr-2 text-slate-700">
                        {t.testType}{t.testSubtype ? ` / ${t.testSubtype}` : ''}
                      </td>
                      <td className="py-1 pr-2 text-slate-600">{t.status}</td>
                      <td className="py-1 text-slate-600">{withEmptyValue(t.assignedToName)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {adResult.analysis_remarks && (
              <p className="text-xs text-slate-500 mb-1">
                <span className="font-medium text-slate-600">Analysis: </span>{adResult.analysis_remarks}
              </p>
            )}
            {adResult.certification_remarks && (
              <p className="text-xs text-slate-500">
                <span className="font-medium text-slate-600">Certification: </span>{adResult.certification_remarks}
              </p>
            )}
            {adResult.received_at && (
              <p className="text-[11px] text-slate-400 mt-1.5">
                Received {dayjs(adResult.received_at).format('DD MMM YYYY, HH:mm')}
              </p>
            )}
          </div>
        )}
  ...
  if (type === 'auto_id' || type === 'carried_id') {
    return (
      <span className="  text-sm text-violet-700 bg-violet-50 px-2 py-0.5 rounded">
