...
function n(val: number | undefined | null, dec = 2): string {
  if (val === undefined || val === null || isNaN(val)) return EMPTY_VALUE_TEXT
...
const TC = ({ val, dec = 2, unit, bold, warn }: { val: number; dec?: number; unit?: string; bold?: boolean; warn?: boolean }) => {
  ...
  return (
    ...
    >
      {display}{display !== EMPTY_VALUE_TEXT && unit ? <span className="opacity-60 ml-0.5 font-normal">{unit}</span> : null}
    </td>
  )
}
    ...
    <div className="space-y-4 text-xs" style={{ fontFamily: 'Calibri, Arial, sans-serif' }}>
      ...
      {!isReady && (
        ...
        >
          ...
          concentrations, DMA limit) to generate the calculation table. Fields you haven't filled in yet show as "{EMPTY_VALUE_TEXT}".
        </div>
      )}
