...
import { projectApi, notebookApi, departmentApi, type Project } from '../../../api/adc'
...
function Field({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="text-sm lg:text-base font-medium text-slate-800">{withEmptyValue(value)}</div>
    </div>
  )
}
...
export default function ProjectInfoTab({ project, projectId }: Props) {
  ...
  const canClose = useCan('adc.project.close')
  const canReopen = useCan('adc.project.reopen')
  const canDeactivate = useCan('adc.project.deactivate')
  ...
  // Only needed to gate "Deactivate Project" client-side (the backend is the
  // real enforcement) — every Notebook under this Project must already be
  // Deactivated.
  const { data: notebooks = [] } = useQuery({
    queryKey: ['adc-project-notebooks-for-lifecycle', projectId],
    queryFn: () => notebookApi.listForProject(projectId).then(r => r.items),
    enabled: canDeactivate,
  })
  const allNotebooksDeactivated = notebooks.length > 0 && notebooks.every(n => n.status === 'DEACTIVATED')

  const invalidateProject = () => {
    qc.invalidateQueries({ queryKey: ['adc-project', projectId] })
  }
  const closeMut = useMutation({
    mutationFn: (password: string) => projectApi.close(projectId, { password }),
    onSuccess: () => { invalidateProject(); message.success('Project closed.') },
  })
  const reopenMut = useMutation({
    mutationFn: (password: string) => projectApi.reopen(projectId, { password }),
    onSuccess: () => { invalidateProject(); message.success('Project reopened.') },
  })
  const deactivateMut = useMutation({
    mutationFn: (password: string) => projectApi.deactivate(projectId, { password }),
      ...
      {editing ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-4 lg:gap-y-5">
            ...
            <Field label="STATUS"      value={<LifecycleStatusTag status={project.status} />} />
          </div>
        ...
        {obsEditing ? (
          ...
          <div>
            ...
            <button
              ...
              className="mt-1.5 text-xs text-violet-500 hover:text-violet-700 flex items-center gap-1"
            >
