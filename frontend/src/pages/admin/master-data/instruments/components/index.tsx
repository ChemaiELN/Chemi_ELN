...
const AdminInstrumentsPage: React.FC = () => {
  ...
  const statusColor = (val: string | null | undefined) => {
    if (!val) return 'default'
    if (val === 'OK') return 'success'
    if (val === 'DUE') return 'warning'
    if (val === 'OVERDUE' || val === 'EXPIRED') return 'error'
    return 'processing'
  }
  ...
  const columns: ColumnsType<LookupInstrument> = [
    ...
    { title: 'Maintenance', dataIndex: 'maintenance_status', key: 'maintenance_status', width: 130,
      render: v => v ? <Tag className={styles.statusTag} color={statusColor(v)}>{v}</Tag> : '—' },
