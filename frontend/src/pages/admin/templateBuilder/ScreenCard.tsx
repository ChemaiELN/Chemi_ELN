...
import RepeatingGroupEditor from './RepeatingGroupEditor'
import type { TemplateField, TemplateScreen, FieldType } from './types'

function FieldRow({
  field, sectionId, screenId, onEdit, onDuplicate, onDelete,
  onEditSubField, onDuplicateSubField, onDeleteSubField, onAddSubField,
}: {
  field: TemplateField
  sectionId: string
  screenId: string
  ...
  onDuplicate: () => void
  onDelete: () => void
  // REPEATING_GROUP only — editing the fields nested inside its sub-screens.
  onEditSubField: (subScreenId: string, field: TemplateField) => void
  onDuplicateSubField: (subScreenId: string, fieldId: string) => void
  onDeleteSubField: (subScreenId: string, fieldId: string) => void
  onAddSubField: (subScreenId: string, type: FieldType) => void
}) {
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          {field.type === 'REPEATING_GROUP' && field.repeatConfig ? (
            <RepeatingGroupEditor
              screens={field.repeatConfig.screens}
              itemLabel={field.repeatConfig.itemLabel ?? 'Item'}
              onEditField={onEditSubField}
              onDuplicateField={onDuplicateSubField}
              onDeleteField={onDeleteSubField}
              onAddField={onAddSubField}
            />
          ) : (
...
interface ScreenCardProps {
  ...
  // REPEATING_GROUP only — editing the fields nested inside its sub-screens.
  onEditSubField: (groupFieldId: string, subScreenId: string, field: TemplateField) => void
  onDuplicateSubField: (groupFieldId: string, subScreenId: string, fieldId: string) => void
  onDeleteSubField: (groupFieldId: string, subScreenId: string, fieldId: string) => void
  onAddSubField: (groupFieldId: string, subScreenId: string, type: FieldType) => void
}
        ...
        <SortableContext items={fieldIds} strategy={verticalListSortingStrategy}>
          {screen.fields.map(field => (
            <FieldRow
              ...
              onEditSubField={(subScreenId, f) => onEditSubField(field.id, subScreenId, f)}
              onDuplicateSubField={(subScreenId, fieldId) => onDuplicateSubField(field.id, subScreenId, fieldId)}
              onDeleteSubField={(subScreenId, fieldId) => onDeleteSubField(field.id, subScreenId, fieldId)}
              onAddSubField={(subScreenId, type) => onAddSubField(field.id, subScreenId, type)}
            />
          ))}
        </SortableContext>
