...
import {
  ...
  CloseCircleOutlined, UploadOutlined, DeleteOutlined,
  WarningOutlined, FileDoneOutlined,
...
import {
  ...
  uploadATRFinalReport, deleteATRFinalReport,
  getUsers,
  type ATRResponse,
  type ATRAttachmentResponse,
  type ATRFinalReportResponse,
...
const STATUS_TAG: Record<string, string> = {
  NEW:       styles.tagStone,
  SUBMITTED: styles.tagSky,
  VERIFIED:  styles.tagTeal,
  COMPLETED: styles.tagEmerald,
  CANCELLED: styles.tagStone,
}
...
const ATRForm: React.FC = () => {
  ...
  const [finalReports, setFinalReports] = useState<ATRFinalReportResponse[]>([])
  ...
  const loadATR = () => {
    ...
    getATR(id)
      .then(a => {
        ...
        setFinalReports(a.final_reports ?? [])
      })
  ...
  // ── Final Reports ─────────────────────────────────────────────────────────
  const finalReportUploadProps: UploadProps = {
    beforeUpload: async (file) => {
      setFrUploading(true)
      try {
        const report = await uploadATRFinalReport(id!, file)
        setFinalReports(prev => [...prev, report])
        message.success(`${file.name} uploaded`)
      } catch (err) {
        message.error(err instanceof Error ? err.message : 'Upload failed')
      } finally {
  ...
  const handleDeleteFinalReport = async (reportId: string) => {
    try {
      await deleteATRFinalReport(id!, reportId)
      setFinalReports(prev => prev.filter(r => r.id !== reportId))
      message.success('Final report deleted')
    } catch (err) {
      message.error(err instanceof Error ? err.message : 'Failed to delete')
    }
  }
  ...
  const finalReportColumns: ColumnsType<ATRFinalReportResponse> = [
    { title: 'File Name', dataIndex: 'filename', key: 'filename',
      render: (v: string) => (
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <FileDoneOutlined style={{ color: '#0f766e' }} />
          {v}
        </span>
      ) },
    { title: 'Size',      dataIndex: 'file_size', key: 'file_size', width: 90,
      render: (v: number | null) => v ? `${Math.round(v / 1024)} KB` : '—' },
    { title: 'Uploaded By', dataIndex: 'uploaded_by', key: 'uploaded_by', width: 130 },
    { title: 'Uploaded At', dataIndex: 'uploaded_at', key: 'uploaded_at', width: 110,
      render: (v: string) => v?.slice(0, 10) },
    {
      title: 'Actions', key: 'actions', width: 72,
      render: (_: unknown, record: ATRFinalReportResponse) => (
        <Popconfirm title="Delete this final report?" onConfirm={() => handleDeleteFinalReport(record.id)}
          okText="Delete" okButtonProps={{ danger: true }}>
          <Button size="small" icon={<DeleteOutlined />} danger type="text" />
        </Popconfirm>
      ),
        ...
        <main className={styles.main}>
          ...
          <div className={styles.pageHeader}>
            <div className={styles.pageHeaderLeft}>
              ...
              <Tag className={`${styles.statusTag} ${STATUS_TAG[atr.status] ?? styles.tagStone}`}>
                {atr.status}
              </Tag>
              {/* v2: version badge */}
            ...
            <Card className={styles.experimentBar}>
              <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'center' }}>
                <div className={styles.expItem}>
                  ...
                  <Text className={styles.expValue} style={{ cursor: 'pointer', color: '#0f766e' }}
          ...
          <div className={styles.tabsCard}>
            <Tabs defaultActiveKey="details" size="small">
              ...
              {/* Tab 3: Final Reports (v2 new) */}
              <TabPane
                tab={
                  <span>
                    <FileDoneOutlined style={{ marginRight: 4 }} />
                    Final Reports{finalReports.length ? ` (${finalReports.length})` : ''}
                  </span>
                }
                key="final-reports"
              >
                <Upload {...finalReportUploadProps}>
                  <div className={styles.finalReportUploadBox}>
                    <UploadOutlined style={{ marginRight: 6 }} />
                    {frUploading ? 'Uploading…' : 'Click to upload a final report'}
                  </div>
                </Upload>
                {finalReports.length === 0 ? (
                  <div className={styles.finalReportEmpty}>
                    No final reports uploaded yet.
                  </div>
                ) : (
                  <Table<ATRFinalReportResponse>
                    columns={finalReportColumns}
                    dataSource={finalReports.map(r => ({ ...r, key: r.id }))}
                    size="small"
                    pagination={false}
                    locale={{ emptyText: 'No final reports uploaded yet.' }}
                    style={{ fontSize: 13 }}
                  />
                )}
              </TabPane>
