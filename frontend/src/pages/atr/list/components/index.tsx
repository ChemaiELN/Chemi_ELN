...
const statusTagProps: Record<string, { className: string }> = {
  NEW:       { className: styles.tagStone },
  SUBMITTED: { className: styles.tagSky },
  VERIFIED:  { className: styles.tagTeal },
  COMPLETED: { className: styles.tagEmerald },
  CANCELLED: { className: styles.tagStone },
}
...
const ATRList: React.FC = () => {
  ...
  const columns: ColumnsType<ATRRecord> = [
    ...
    },
    {
      ...
      key: 'status',
      render: (val: string) => {
        const props = statusTagProps[val] ?? { className: styles.tagStone }
        return <Tag className={`${styles.statusTag} ${props.className}`}>{val}</Tag>
      },
    },
    {
      title: 'Raised By',
          ...
          <div className={styles.filterCard}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 4 }}>
              ...
              <Switch
                ...
                style={{ background: latestOnly ? '#0f766e' : undefined }}
              />
            </div>
          </div>
