...
// Mirrors AdcReportsPage.tsx — downloads a Word (.docx) report for each
// APPROVED CGT experiment (chemist signed -> HOD signed).
export default function CgtReportsPage() {
  const [search, setSearch] = useState('')
  const [page, setPage]     = useState(1)
  const [downloading, setDl] = useState<string | null>(null)

  const { data, isLoading } = useQuery({
    queryKey: ['cgt-reports-approved', search, page],
  ...
  const handleDownload = async (exp: CgtExperimentListItem) => {
    setDl(exp.id)
    try {
      const { blob, filename } = await cgtExperimentApi.downloadReport(exp.id)
  ...
  const columns = [
    ...
    {
      title: 'Experiment', dataIndex: 'title', key: 'title',
      render: (v: string) => <span className="text-[13px] font-medium text-slate-800">{v}</span>,
    },
    {
    ...
    {
      title: 'Notebook', key: 'notebook', width: 160,
      render: (_: unknown, row: CgtExperimentListItem) => (
        <div className="text-[12px]">
    ...
    {
      title: 'Approved At', dataIndex: 'approved_at', key: 'approved_at', width: 160,
      render: (v: string) => v ? (
    ...
    {
      title: 'Report', key: 'report', width: 100,
      render: (_: unknown, row: CgtExperimentListItem) => (
