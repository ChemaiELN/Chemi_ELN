...
function AuditTab({ entityId }: { entityId: number }) {
  ...
  const filters = useMemo(
    () => ({ entity_type: 'inv_instrument_catalogue', entity_id: entityId }),
    [entityId],
  )
...
export default function InstrumentDetailPage() {
  ...
  const summary = (
    <div className="glass-card rounded-lg p-4 grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1">
      ...
      <Field label="Calibration Type" value={item.calibration_type} />
      <Field label="Calibration Frequency" value={item.calibration_frequency_value != null ? `${item.calibration_frequency_value} ${item.calibration_frequency_unit ?? ''}`.trim() : null} />
