...
import {
  Button, Empty, Table, Modal, Form, Input, Radio, Checkbox, InputNumber, Select,
  message, Space,
} from 'antd'
import type { ColumnsType } from 'antd/es/table'
import { ArrowLeft, PlayCircle, StopCircle, CheckCircle2, ShieldCheck, RotateCcw, Plus, Trash2, Truck } from 'lucide-react'
import { StatusTag } from '../../components/ui/StatusTag'
import BrandSpinner from '../../components/ui/BrandSpinner'
import {
  workOrderApi, sparePartApi, measurementMasterApi, gatePassApi, manufacturerApi,
  type WorkOrderDetail, type ChecklistItem, type SparePart, type MeasurementMaster, type CalibrationReference,
  type GatePass, type Manufacturer,
} from '../../api/inventory'
...
function ChecklistTable({ wo, editable, onChanged }: { wo: WorkOrderDetail; editable: boolean; onChanged: () => void }) {
  ...
  const renderObservation = (item: ChecklistItem) => {
    ...
    if (item.data_type === 'MULTIPLE_SELECTION') {
      // Stored as one comma-separated string in `observation`. This used to render
      // as a Radio.Group, so only a single value could ever be recorded on an item
      // that explicitly allows several.
      ...
      return (
        <Checkbox.Group
          ...
          onChange={vals => {
            // Save in the item's own option order so the stored string is stable
            // regardless of the order the boxes were clicked.
            const next = options.filter(o => (vals as string[]).includes(o)).join(', ')
            setVal(item.id, { observation: next })
            saveRow(item.id, { observation: next })
          }}
        />
      )
    }
    if (item.data_type === 'SINGLE_SELECTION') {
      const options = item.options ?? []
      return (
...
function ESignModal({ open, title, needsMaintenanceType, failedCount, onClose, onSubmit, saving }: {
  open: boolean; title: string; needsMaintenanceType?: boolean; failedCount?: number
  onClose: () => void
  onSubmit: (v: { password: string; comment: string; maintenance_type?: string; acknowledge_deviation?: boolean }) => void
  saving: boolean
}) {
  ...
  return (
    <Modal title={title} open={open} closable={false} onCancel={onClose} onOk={() => form.submit()} confirmLoading={saving} width={440} centered destroyOnHidden {...glassModalProps}>
      <Form form={form} layout="vertical" onFinish={onSubmit}>
        ...
        {/* An out-of-tolerance reading can't be signed off as if it passed — the
            backend rejects approval unless this is explicitly acknowledged. */}
        {!!failedCount && (
          <>
            <div className="mb-3 px-3 py-2 rounded-md bg-red-50 border border-red-200 text-[13px] text-red-700">
              {failedCount} calibration reading{failedCount > 1 ? 's are' : ' is'} outside tolerance (FAIL).
              This instrument is not in spec — approving it records a deviation.
            </div>
            <Form.Item
              ...
              rules={[{
                validator: (_, val) => val
                  ? Promise.resolve()
                  : Promise.reject(new Error('You must acknowledge the deviation to approve.')),
              }]}
            >
              <Checkbox>I approve this as an acknowledged deviation</Checkbox>
            </Form.Item>
          </>
        )}
...
// ── Create Gate Pass (from external calibration WO) modal ─────────────────────
function GatePassModal({ open, workOrderId, onClose, onCreated }: {
  open: boolean; workOrderId: number; onClose: () => void; onCreated: (gpId: number) => void
}) {
  const [form] = Form.useForm()
  const [saving, setSaving] = useState(false)
  const [vendors, setVendors] = useState<Manufacturer[]>([])
  useEffect(() => {
    if (open) { form.resetFields(); manufacturerApi.list({ limit: 200 }).then(setVendors).catch(() => {}) }
  }, [open, form])

  const submit = async (v: { manufacturer_id: number; remarks?: string }) => {
    setSaving(true)
    try {
      const gp = await gatePassApi.fromWorkOrder({ work_order_id: workOrderId, manufacturer_id: v.manufacturer_id, remarks: v.remarks })
      message.success(`Gate Pass ${gp.gp_number} created`)
      onCreated(gp.id)
    } catch (e: unknown) { message.error((e as Error).message) }
    finally { setSaving(false) }
  }
  ...
  return (
    <Modal title="Create Returnable Gate Pass" open={open} closable={false} onCancel={onClose} onOk={() => form.submit()} confirmLoading={saving} width={480} centered destroyOnHidden {...glassModalProps}>
      <p className="text-[13px] text-slate-500 mb-3">Send this instrument out to an external calibration vendor. A returnable gate pass (RGP) will be created and linked to this work order.</p>
      <Form form={form} layout="vertical" onFinish={submit}>
        <Form.Item name="manufacturer_id" label="Calibration Vendor" rules={[{ required: true }]}>
          <Select showSearch optionFilterProp="label" placeholder="Select vendor"
...
export default function WorkOrderExecutionPage() {
  ...
  const [linkedGp, setLinkedGp] = useState<GatePass | null>(null)
  ...
  const load = useCallback(() => {
    ...
    gatePassApi.byWorkOrder(Number(id)).then(setLinkedGp).catch(() => {})
  }, [id])
  useEffect(() => { load() }, [load])

  if (loading) return <div className="flex items-center justify-center h-64"><BrandSpinner fullScreen={false} label="Loading work order…" /></div>
  ...
  const doESign = async (v: { password: string; comment: string; maintenance_type?: string; acknowledge_deviation?: boolean }) => {
    setEsignSaving(true)
    try {
  ...
  // Readings outside tolerance. Approval requires an explicit deviation
  // acknowledgement when this is non-zero (enforced server-side too).
  const failedRefCount = (wo.calib_references ?? []).filter(r => r.status === 'FAIL').length
  ...
  <StatusTag color={STATUS_COLOR[wo.status] ?? "default"}>
    {wo.status.replace(/_/g, " ")}
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {isExternalCalibration && (linkedGp
            ? <Button icon={<Truck size={14} />} onClick={() => navigate(`/inventory/gate-passes/${linkedGp.id}`)}>Gate Pass: {linkedGp.gp_number}</Button>
            : <Button icon={<Truck size={14} />} onClick={() => setGpModalOpen(true)}>Create Gate Pass</Button>
          )}
        </div>
      </div>
      ...
      <ESignModal
        ...
        failedCount={esign === 'approve' ? failedRefCount : 0}
      ...
      <GatePassModal open={gpModalOpen} workOrderId={wo.id} onClose={() => setGpModalOpen(false)} onCreated={(gpId) => { setGpModalOpen(false); navigate(`/inventory/gate-passes/${gpId}`) }} />
    </div>
  )
}
