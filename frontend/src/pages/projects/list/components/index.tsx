...
import { Table, Button, Input, Select, Tag, message, Modal, Form, DatePicker } from 'antd'
import {
  HomeOutlined,
  PlusOutlined,
  ExportOutlined,
...
export default function ProjectsListPage() {
  ...
  const handleSearch = () => { /* filtering is reactive */ }
  ...
  const columns: ColumnsType<Project> = [
    { title: '#',               dataIndex: 'no',            key: 'no',            width: 50 },
    {
      ...
      render: (name: string, record: Project) => (
        <a
          style={{ color: '#0f766e', fontWeight: 500, cursor: 'pointer' }}
    ...
    { title: 'Code',            dataIndex: 'code',          key: 'code'           },
    {
      ...
      render: (type: Project['type']) => (
        <Tag className={type === 'Internal' ? styles.typeInternal : styles.typeExternal}>
          {type}
        </Tag>
      ),
    },
    ...
    { title: 'Created By',       dataIndex: 'createdBy',     key: 'createdBy'      },
    {
      ...
      render: (status: string) => (
        <Tag className={`${styles.statusTag} ${STATUS_CLASS[status] ?? ''}`}>{STATUS_LABEL[status] ?? status}</Tag>
      ),
    },
    {
      title: 'Actions',
