...
export async function getAuditLog(params?: {
  ...
  const { data } = await client.get('/api/admin/audit', { params })
...
export async function getRoles(): Promise<Role[]> {
  ...
  return (data as PaginatedResponse<Role>).items ?? []
}

/** v2: GET /api/roles/ — lightweight list for dropdowns */
export async function getRolesList(): Promise<RoleShort[]> {
  const { data } = await client.get('/api/roles/')
  return data as RoleShort[]
}
...
export interface ExperimentSummary {
  id: string
  code: string          // OQ/R1/S1/E00001
  full_code: string     // OQ/R1/S1/E00001/001
  version: number
  title: string
  status: string        // DRAFT | SUBMITTED | VERIFIED | APPROVED | REJECTED | UNLOCKED | VOID
  is_latest_version: boolean
  /** v2: whether QA has highlighted this experiment */
...
// ─── Experiment Steps (v2 new) ────────────────────────────────

export interface ExperimentStepResponse {
  id: string
...
export interface ExperimentStepUpdate {
  procedure_text?: string
  observation_text?: string
  qty?: string
  temperature?: string
...
// ─── Experiment Equipment (v2 new) ────────────────────────────

export interface ExperimentEquipmentResponse {
  id: string
  experiment_id: string
...
export interface ExperimentEquipmentCreate {
...
// ─── Experiment Inputs (v2 enhanced) ─────────────────────────

export interface ExperimentInputResponse {
  id: string
  experiment_id: string
  sort_order: number
  material_name: string
  cas_no: string | null
  mol_weight: string | null
  quantity: string | null
  unit: string | null
  moles: string | null
  mole_ratio: string | null
  ...
  required_qty_unit: string | null
  density: string | null
  strength: string | null
  ww_ratio: string | null
  molarity: string | null
  remarks: string | null
}

export interface ExperimentInputCreate {
  sort_order?: number
  material_name: string
  cas_no?: string
  mol_weight?: string
...
// ─── Experiment Parameters (v2 formula engine) ────────────────

export interface ExperimentParameterResponse {
  id: string
  experiment_id: string
  sort_order: number
  name: string          // kept for backward compat (v1 field)
  value: string | null  // kept for backward compat (v1 field)
  unit: string | null   // kept for backward compat (v1 field)
  /** v2 fields */
  code: string | null
  /** INPUT or OUTPUT */
  input_output: string
  /** USER ENTERED or FORMULA */
  user_entered_or_formula: string
  /** NUMBER | TEXT | DATE */
  param_type: string
  formula_expression: string | null
  /** Calculated/stored numeric value */
...
export interface ExperimentTLCResponse {
  id: string
  experiment_id: string
  solvent_system: string | null
  rf_starting_material: string | null
  rf_product: string | null
  visualization: string | null
  notes: string | null
  recorded_at: string | null
  /** v2: path to TLC drawing image */
  drawing_path: string | null
}

export interface ExperimentTLCCreate {
  solvent_system?: string
  rf_starting_material?: string
  rf_product?: string
...
// ─── Experiment History (v2 enhanced) ─────────────────────────

export interface HistoryResponse {
  id: string
  experiment_id: string
  version_experiment_id: string | null
  action: string
  action_by: string
  action_at: string
  ...
  save_comments: string | null
  /** Denormalized display name for action_by (may be absent from some endpoints) */
...
export interface ExperimentCreate {
  ...
  actual_yield?: string
  yield_pct?: string
  /** v2: code of reference / cloned-from experiment */
  reference_exp_code?: string
}
...
export interface ExperimentUpdate {
  ...
  precautions?: string
  /**
   * @deprecated scheme_mol removed in v2 — use steps/procedure instead.
   * Kept here only so existing editor code compiles; will be removed in Phase 3.
   */
  ...
  /** v2 */
  reference_exp_code?: string
  /** v2: comments entered when saving a draft */
  save_comments?: string
  /** v2: path to TLC drawing */
  tlc_drawing_path?: string | null
  route_id?: string | null
  stage_id?: string | null
}
...
export interface ExperimentResponse extends ExperimentSummary {
  ...
  theoretical_yield: string | null
  actual_yield: string | null
  yield_pct: string | null
  submitted_by: string | null
  submitted_at: string | null
  /** v2: the user this experiment is submitted to (verification request flow) */
  submitted_to: string | null
  submitted_to_at: string | null
  /** Denormalized display name for submitted_to (may be absent from some endpoints) */
  ...
  revision_note: string | null
  /** v2 */
  post_verification_remarks: string | null
  improvement_suggestions: string | null
  save_comments: string | null
  reference_exp_code: string | null
  tlc_drawing_path: string | null
  /**
   * @deprecated scheme_mol removed in v2.
   * Kept for backward compat until Phase 3 cleans up the editor.
   */
  scheme_mol?: string | null
  /** v2: QA-set highlight flag */
  highlight_comments: string | null
  inputs: ExperimentInputResponse[]
  parameters: ExperimentParameterResponse[]
  tlc_records: ExperimentTLCResponse[]
  attachments: ExperimentAttachmentResponse[]
  /** v2 */
  steps: ExperimentStepResponse[]
  equipment: ExperimentEquipmentResponse[]
}
...
export async function getExperiments(params?: {
  ...
  const { data } = await client.get('/api/experiments', { params: { latest_only: true, ...params } })
...
export async function createExperiment(body: ExperimentCreate): Promise<ExperimentResponse> {
  const { data } = await client.post('/api/experiments/', body)
  return data as ExperimentResponse
}
...
export async function verifyExperiment(id: string, password?: string): Promise<ExperimentResponse> {
  const { data } = await client.post(`/api/experiments/${id}/verify`, password ? { password } : undefined)
  return data as ExperimentResponse
}
...
export async function reviseExperiment(id: string): Promise<ExperimentResponse> {
  const { data } = await client.post(`/api/experiments/${id}/revise`)
  return data as ExperimentResponse
}

export async function voidExperiment(id: string, password?: string): Promise<ExperimentResponse> {
  const { data } = await client.post(`/api/experiments/${id}/void`, password ? { password } : undefined)
  return data as ExperimentResponse
}

export async function unlockExperiment(id: string): Promise<ExperimentResponse> {
  const { data } = await client.post(`/api/experiments/${id}/unlock`)
  return data as ExperimentResponse
}

export async function newVersionExperiment(id: string, revision_note: string): Promise<ExperimentResponse> {
  const { data } = await client.post(`/api/experiments/${id}/new-version`, { revision_note })
  return data as ExperimentResponse
}

// ─── Experiment Inputs CRUD ───────────────────────────────────

export async function addExperimentInput(expId: string, body: ExperimentInputCreate): Promise<ExperimentInputResponse> {
  const { data } = await client.post(`/api/experiments/${expId}/inputs`, body)
  return data as ExperimentInputResponse
}

export async function updateExperimentInput(expId: string, inputId: string, body: Partial<ExperimentInputCreate>): Promise<ExperimentInputResponse> {
  const { data } = await client.patch(`/api/experiments/${expId}/inputs/${inputId}`, body)
  return data as ExperimentInputResponse
}

export async function deleteExperimentInput(expId: string, inputId: string): Promise<void> {
  await client.delete(`/api/experiments/${expId}/inputs/${inputId}`)
}

// ─── Experiment Parameters CRUD ───────────────────────────────

export async function addExperimentParameter(expId: string, body: ExperimentParameterCreate): Promise<ExperimentParameterResponse> {
  const { data } = await client.post(`/api/experiments/${expId}/parameters`, body)
  return data as ExperimentParameterResponse
}

export async function updateExperimentParameter(expId: string, paramId: string, body: Partial<ExperimentParameterCreate>): Promise<ExperimentParameterResponse> {
  const { data } = await client.patch(`/api/experiments/${expId}/parameters/${paramId}`, body)
  return data as ExperimentParameterResponse
}

export async function deleteExperimentParameter(expId: string, paramId: string): Promise<void> {
  await client.delete(`/api/experiments/${expId}/parameters/${paramId}`)
}

// ─── Experiment Steps CRUD (v2 new) ───────────────────────────

export async function addExperimentStep(expId: string, body: ExperimentStepCreate): Promise<ExperimentStepResponse> {
  const { data } = await client.post(`/api/experiments/${expId}/steps`, body)
  return data as ExperimentStepResponse
}

export async function updateExperimentStep(expId: string, stepId: string, body: ExperimentStepUpdate): Promise<ExperimentStepResponse> {
  const { data } = await client.patch(`/api/experiments/${expId}/steps/${stepId}`, body)
  return data as ExperimentStepResponse
}

export async function deleteExperimentStep(expId: string, stepId: string): Promise<void> {
  await client.delete(`/api/experiments/${expId}/steps/${stepId}`)
}

export async function uploadExperimentStepAttachment(expId: string, stepId: string, file: File, password?: string): Promise<ExperimentStepResponse> {
  const form = new FormData()
  form.append('file', file)
  if (password) form.append('password', password)
  const { data } = await client.post(`/api/experiments/${expId}/steps/${stepId}/attachment`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  return data as ExperimentStepResponse
}

// ─── Experiment Equipment CRUD (v2 new) ───────────────────────

export async function addExperimentEquipment(expId: string, body: ExperimentEquipmentCreate): Promise<ExperimentEquipmentResponse> {
  const { data } = await client.post(`/api/experiments/${expId}/equipment`, body)
  return data as ExperimentEquipmentResponse
}

export async function deleteExperimentEquipment(expId: string, equipId: string): Promise<void> {
  await client.delete(`/api/experiments/${expId}/equipment/${equipId}`)
}

// ─── Experiment TLC ───────────────────────────────────────────

export async function addExperimentTLC(expId: string, body: ExperimentTLCCreate): Promise<ExperimentTLCResponse> {
  const { data } = await client.post(`/api/experiments/${expId}/tlc`, body)
  return data as ExperimentTLCResponse
}
...
export async function getExperimentComments(expId: string): Promise<CommentResponse[]> {
  const { data } = await client.get(`/api/experiments/${expId}/comments`)
  return data as CommentResponse[]
}

export async function addExperimentComment(expId: string, body: CommentCreate): Promise<CommentResponse> {
  const { data } = await client.post(`/api/experiments/${expId}/comments`, body)
  return data as CommentResponse
}
...
export async function getExperimentAttachments(expId: string): Promise<ExperimentAttachmentResponse[]> {
  const { data } = await client.get(`/api/experiments/${expId}/attachments`)
...
export async function uploadExperimentAttachment(expId: string, file: File, password?: string): Promise<ExperimentAttachmentResponse> {
  ...
  form.append('file', file)
  if (password) form.append('password', password)
  const { data } = await client.post(`/api/experiments/${expId}/attachments`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  return data as ExperimentAttachmentResponse
}
...
export async function deleteExperimentAttachment(expId: string, attId: string): Promise<void> {
  await client.delete(`/api/experiments/${expId}/attachments/${attId}`)
}
...
export async function exportExperimentPDF(expId: string, params?: ExportPDFParams): Promise<Blob> {
  const { data } = await client.get(`/api/experiments/${expId}/export-pdf`, {
    params,
    responseType: 'blob',
  })
  return data as Blob
...
/** v2: upload a final QA report file to an ATR */
export async function uploadATRFinalReport(atrId: string, file: File): Promise<ATRFinalReportResponse> {
  const form = new FormData()
  form.append('file', file)
  const { data } = await client.post(`/api/atr/${atrId}/final-reports`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  return data as ATRFinalReportResponse
}

/** v2: delete a final report file from an ATR */
export async function deleteATRFinalReport(atrId: string, reportId: string): Promise<void> {
  await client.delete(`/api/atr/${atrId}/final-reports/${reportId}`)
}
...
export async function approveUnlockRequest(id: string, review_note?: string): Promise<UnlockRequestResponse> {
  const { data } = await client.post(`/api/unlock-requests/${id}/approve`, { review_note })
  return data as UnlockRequestResponse
}

export async function rejectUnlockRequest(id: string, review_note?: string): Promise<UnlockRequestResponse> {
  const { data } = await client.post(`/api/unlock-requests/${id}/reject`, { review_note })
  return data as UnlockRequestResponse
}
...
export async function searchExperimentsByParameter(params: {
  code?: string
  value_min?: number
  value_max?: number
  page?: number
  ...
  const { data } = await client.get('/api/search/experiments/by-parameters', { params })
  return data as PaginatedResponse<SearchExperimentResult>
}
