...
import { getExperiments, createExperiment, getNotebooks, type ExperimentSummary as APIExperiment } from '@/utilities/chemiaApi';
import {
  Table,
  Button,
  Input,
...
function mapAPIExperiment(e: APIExperiment, idx: number): Experiment {
  return {
    ...
    createdBy:            e.creator_name ?? e.created_by,
...
const statusClassMap: Partial<Record<string, string>> = {
...
const buildColumns = (
  ...
  const base: ColumnsType<Experiment> = [
    ...
    {
      title: 'Full Code', dataIndex: 'fullCode', key: 'fullCode',
      render: (val: string) => (
        <span style={{ fontFamily: 'monospace', fontWeight: 500 }}>{val}</span>
      ),
    },
    { title: 'Version', dataIndex: 'version', key: 'version', width: 72,
      render: (v: number) => <span style={{ fontFamily: 'monospace' }}>{String(v).padStart(3, '0')}</span> },
    {
      title: 'Status', dataIndex: 'status', key: 'status', width: 110,
      render: (val: string) => (
        <Tag className={`${styles.statusTag} ${statusClassMap[val] ?? ''}`}>
      ...
      render: (_: unknown, record: Experiment) => (
        <Tooltip title="View">
          <Button
            ...
            icon={<EyeOutlined />}
            size="small"
            onClick={() => navigate(`/experiments/${record.key}`)}
          />
        </Tooltip>
      ),
    },
...
const ExperimentsListPage: React.FC = () => {
  ...
  const openCreate = () => {
    ...
    getNotebooks({ page_size: 100 })
      .then(r => setNbOptions(r.items.map(nb => ({ value: nb.id, label: `${nb.code} — ${nb.title}` }))))
      .catch(() => {});
    setCreateOpen(true);
  };
  ...
  const handleCreate = async (values: { notebook_id: string; title: string }) => {
    ...
    try {
      const exp = await createExperiment({ notebook_id: values.notebook_id, title: values.title });
  ...
  const loadExperiments = () => {
    ...
    getExperiments({ page_size: 100 })
      .then((resp) => setAllData(resp.items.map(mapAPIExperiment)))
  ...
  const columns = useMemo(() => buildColumns(viewMode, navigate), [viewMode]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data;
  }, [allData, viewMode, pendingSub, searchText]);
