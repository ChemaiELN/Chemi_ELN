...
import { Table, Button, Select, Input, Badge, Breadcrumb, message, Modal, Form, Tooltip, Spin } from 'antd'
import {
  ...
  const handleCreate = async (values: Record<string, string>) => {
    ...
    try {
      ...
      if (values.template_id) {
        localStorage.setItem(NB_TMPL_KEY(nb.id), values.template_id)
      }
      message.success('Notebook created')
      setCreateOpen(false)
      navigate(`/notebooks/${nb.id}/overview${values.template_id ? '?newExp=1' : ''}`)
    } catch (err) {
    ...
    <div className={styles.page}>
            ...
            <div className={styles.chemCard}>
              <div className={styles.chemCardHeader}>
                <div className={styles.tableCardTitle}>
                  My Notebooks
                  <Badge count={notebooks.length} style={{ backgroundColor: '#f5f5f4', color: '#57534e', boxShadow: 'none', fontWeight: 600, fontSize: 11 }} />
                </div>
              </div>
              {loading ? (
      ...
      <Modal title="New Notebook" open={createOpen}
        ...
        <Form form={createForm} layout="vertical" onFinish={handleCreate} requiredMark={false} style={{ marginTop: 12 }}>
          ...
          <Form.Item name="template_id" label="Experiment Template (optional)"
            extra="Scientists will follow this template's screens when recording experiments.">
            <Select
              allowClear
              placeholder="No template — free-form experiments"
