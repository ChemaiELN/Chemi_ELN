...
const CHIP_CLASS: Record<string, string> = {
  'NOT STARTED': styles.chipNotStarted,
  'ON TRACK':    styles.chipOnTrack,
  'AT RISK':     styles.chipAtRisk,
  'COMPLETED':   styles.chipCompleted,
  'DELAYED':     styles.chipDelayed,
}
...
export default function ProjectMilestonesPage() {
  ...
  const columns: ColumnsType<MilestoneResponse> = [
    ...
    },
    {
      ...
      render: (s: string) => <Tag className={`${styles.statusTag} ${STATUS_CLASS[s] ?? ''}`}>{s}</Tag>,
    },
    {
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div className={styles.summaryRow}>
              ...
              {statusCounts.map(({ status, count }) => (
                <Tag key={status} className={`${styles.chipTag} ${CHIP_CLASS[status]}`}>
