...
import {
  ...
  getExperiments, getUsers, updateProject,
...
const EXP_STATUS_CLASS: Record<string, { bg: string; color: string }> = {
  ...
  UNLOCKED:  { bg: '#fffbeb', color: '#92400e' },
...
export default function ProjectOverviewPage() {
  ...
  const statusStyle = STATUS_TAG[project.status] ?? STATUS_TAG['ACTIVE']
  ...
  const expColumns: ColumnsType<ExperimentSummary> = [
      ...
      render: (v: string | null, r: ExperimentSummary) => v ?? r.created_by },
    {
      ...
      render: (v: string) => {
        const s = EXP_STATUS_CLASS[v] ?? { bg: '#f5f5f4', color: '#78716c' }
        return <Tag style={{ background: s.bg, color: s.color, border: 'none', fontWeight: 600, fontSize: 11 }}>{v}</Tag>
      },
    },
            ...
            <div className={styles.card}>
              ...
              <div className={styles.infoRow}>
                <div className={styles.infoLine}>
                  ...
                  <span className={styles.infoVal} style={{ fontFamily: 'monospace' }}>{project.code}</span>
                </div>
                <div className={styles.infoLine}>
                ...
                <div className={styles.infoLine}>
                  ...
                  <span className={styles.infoVal}>
                    <Tag style={{ background: statusStyle.bg, color: statusStyle.color, borderColor: statusStyle.border, fontWeight: 600 }}>
                      {project.status}
                    </Tag>
                  </span>
                </div>
            ...
            <div className={styles.card}>
              <div className={styles.cardTitleRow}>
                ...
                <Button type="text" size="small" icon={<PlusOutlined />}
                  style={{ color: '#0f766e' }} onClick={openAddMember} />
              </div>
              {members.length === 0
                  ...
                  <div className={styles.memberList}>
                    {members.map(m => (
                      <div key={m.user_id} className={styles.memberRow}>
                        <Avatar size={28} icon={<UserOutlined />} style={{ background: '#0d9488' }} />
